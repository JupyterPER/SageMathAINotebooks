...
function enhanceSageCellsWithGlobalAutocomplete() {
    ...
    const staticCompletions = [
        ...
        { text: "desolve_rk4(", displayText: "desolve_rk4(equation, y, x, ics) - Numerical Runge-Kutta" },
         ...
         // === SNIPPETS (add these at the top for priority) ===
        { 
            text: "for", 
            displayText: "for - For loop snippet",
            snippet: "for ${1:item} in ${2:iterable}:\n    ${0:pass}"
        },
        { 
            text: "if", 
            displayText: "if - If statement snippet",
            snippet: "if ${1:condition}:\n    ${0:pass}"
        },
        { 
            text: "elif", 
            displayText: "elif - Elif statement snippet",
            snippet: "elif ${1:condition}:\n    ${0:pass}"
        },
        { 
            text: "def", 
            displayText: "def - Function definition snippet",
            snippet: "def ${1:function_name}(${2:args}):\n    ${0:pass}"
        },
        { 
            text: "class", 
            displayText: "class - Class definition snippet",
            snippet: "class ${1:ClassName}:\n    def __init__(self, ${2:args}):\n        ${0:pass}"
        },
        { 
            text: "while", 
            displayText: "while - While loop snippet",
            snippet: "while ${1:condition}:\n    ${0:pass}"
        },
        { 
            text: "try", 
            displayText: "try - Try-except block snippet",
            snippet: "try:\n    ${1:pass}\nexcept ${2:Exception} as ${3:e}:\n    ${0:pass}"
        },
        { 
            text: "with", 
            displayText: "with - Context manager snippet",
            snippet: "with ${1:expression} as ${2:variable}:\n    ${0:pass}"
        },
        { 
            text: "lambda", 
            displayText: "lambda - Lambda function snippet",
            snippet: "lambda ${1:args}: ${0:expression}"
        },
        { 
            text: "plot", 
            displayText: "plot - SageMath plot snippet",
            snippet: "plot(${1:function}, (${2:x}, ${3:xmin}, ${4:xmax}))"
        },
        { 
            text: "matrix", 
            displayText: "matrix - Matrix definition snippet",
            snippet: "matrix([[${1:a}, ${2:b}], [${3:c}, ${4:d}]])"
        },
        
        // Your existing completions continue here...
        { text: "def", displayText: "def function_name(arguments): - Define a function" },
    ...
    function extractUserDefinedVariablesFromContent(content, cellId) {
        ...
        // Add these patterns to extractUserDefinedVariablesFromContent:

        // List comprehension variables: [x for x in ...]
        const patternListComp = /\[([a-zA-Z_][a-zA-Z0-9_]*)\s+for\s+\1\s+in\s+/g;

        // Dictionary comprehension: {k: v for k, v in ...}
        const patternDictComp = /{([^}]+)\s+for\s+([^}]+)\s+in\s+/g;

        // With statement: with open(...) as f:
        const patternWith = /^\s*with\s+.+\s+as\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*:/;

        // Class definitions
        const patternClass = /^\s*class\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*[\(:]/ ;

        // Tuple unpacking: a, b = (1, 2)
        const patternTupleUnpack = /^\s*([a-zA-Z_][a-zA-Z0-9_]*(?:\s*,\s*[a-zA-Z_][a-zA-Z0-9_]*)+)\s*=/;
    ...
    function setupEnhancer() {
        ...
        if (!CodeMirror.hint) {
            ...
            script.onload = function() {
                ...
                cssLink.href = 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.65.16/addon/hint/show-hint.min.css';
...
// Add this new function for snippet expansion
function expandSnippet(cm, snippet, from, to) {
    ...
    snippetText = snippetText.replace(/\$\{(\d+):([^}]*)\}/g, '$2');
    
    // Insert the snippet
    const startPos = from;
    cm.replaceRange(snippetText, from);
    ...
    if (placeholders.length > 0) {
        ...
        let currentCh = startPos.ch;
        let searchText = snippet.substring(0, snippet.indexOf(firstPlaceholder.matchText));
        ...
        cm.setSelection(
            ...
            { line: currentLine, ch: currentCh + firstPlaceholder.text.length }
        );
...
// Modify the registerAutocompleteHelper function
function registerAutocompleteHelper() {
    if (CodeMirror.hint && !CodeMirror.hint.__sageHelperInstalled) {
        CodeMirror.registerHelper("hint", "python", function(editor, options) {
            const cursor = editor.getCursor();
            ...
            // Add hint function to handle snippet expansion
            const hintList = uniqueCompletions.map(completion => {
                return {
                    text: completion.text,
                    displayText: completion.displayText,
                    hint: function(cm, data, completion) {
                        if (completion.snippet) {
                            // It's a snippet - expand it
                            expandSnippet(cm, completion.snippet, data.from, data.to);
                        } else {
                            // Normal completion
                            cm.replaceRange(completion.text, data.from, data.to);
                        }
                    },
                    snippet: completion.snippet // Pass through snippet data
                };
            });
