...
function makeMenu() {
    ...
    var t = "de" == e ? "Code ausblenden/einblenden" : "Show / Hide Code",
      ...
      l = '<div id="navbar">' + ('Exec' == playerConfig.panes ? "" : s + o) + '<a href="#" role="button" class="btn btn-primary" onclick="toggleInput()" title="Show/Hide Code">' + iconDictionaryNavbar["showHideCode"] + '</a>\n  <a href="#" role="button" class="btn btn-primary" onclick="saveHtml()" title="Save">' + iconDictionaryNavbar["saveNotebook"] + "</a>" + '\n  </div>';
...
function saveHtml() {
    ...
    const currentAiCellsHiddenState = aiCellsHidden;
    ...
    const htmlContent = "<!DOCTYPE html>\n<html>\n<head>" +
        ...
        '<script src="https://cdn.jsdelivr.net/gh/JupyterPER/SageMathAINotebooks@main/js/nbrunner6.js"></script>\n' +
        ...
        '  let aiCellsHidden = ' + currentAiCellsHiddenState + ';\n' +
        '  makeMenu();\n' +
        '  localize();\n' +
        '  loadStatus();\n' +
...
function toggleEditMode() {
    ...
    if (editCellsButton) {
        ...
        editCellsButton.title = editMode ? 'Edit Mode' : 'View Mode';
    }
}
...
function addControlPanel() {
    ...
    exportButton.title = 'Export TXT Backup';
    ...
    importButton.title = 'Import TXT Backup';
    ...
    toggleAiButton.id = 'toggleAiCellsButton';
    toggleAiButton.textContent = aiCellsHidden ? 'Show AI Cells' : 'Hide AI Cells'; // Initial text based on state
    toggleAiButton.title = aiCellsHidden ? 'Show cells generated by AI' : 'Hide cells generated by AI';
...
const iconDictionaryNavbar = {
    ...
    saveNotebook: `<svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-device-floppy"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M6 4h10l4 4v10a2 2 0 0 1 -2 2h-12a2 2 0 0 1 -2 -2v-12a2 2 0 0 1 2 -2" /><path d="M12 14m-2 0a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /><path d="M14 4l0 4l-6 0l0 -4" /></svg>`,
    toggleNavBar: `<svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-menu-2"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M4 6l16 0" /><path d="M4 12l16 0" /><path d="M4 18l16 0" /></svg>`,
    ...
    exportCells: `<svg xmlns="http://www.w3.org/2000/svg" width="48" height="24" viewBox="0 0 48 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    <!-- First icon (TXT file) -->
    <g transform="translate(0,0)">
    <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
    <path d="M14 3v4a1 1 0 0 0 1 1h4" />
    ...
    <!-- Second icon (Arrow big up) -->
    <g transform="translate(24,0)">
    <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
    <path d="M9 20v-8h-3.586a1 1 0 0 1 -.707 -1.707l6.586 -6.586a1 1 0 0 1 1.414 0l6.586 6.586a1 1 0 0 1 -.707 1.707h-3.586v8a1 1 0 0 1 -1 1h-4a1 1 0 0 1 -1 -1z" fill="none"/>
    </g>
    </svg>`,
...
function formatAndLoadCodeIntoCell(cell, aiCommand, currentModel, apiKey, currentLanguage) {
    ...
    if (!codeMirror) {
        console.error('CodeMirror instance was not found');
        return;
    }
    ...
    // Replace all newlines with \n for proper Python raw string formatting
    previousCode = String(previousCode).replace(/'/g, '"').replace(/\n/g, '\\n');

    const formattedCode = `# -START OF AI CELL-
AIprompt = r'''
${currentQuery}
'''
AIlanguage = '${currentLanguage}'; AImodel='${currentModel}'; AIurl = 'https://raw.githubusercontent.com/JupyterPER/SageMathAINotebooks/refs/heads/main/AIcommandsMistral%20NB%20player.py'; load(AIurl); previous_code = r'''${previousCode}'''; AIanswer = ${aiCommand}(language=AIlanguage, model=AImodel, NBplayer_code=previous_code+f'In[FOCUS]: {AIprompt}', api_key='${apiKey}'); print(AIanswer)
# -END OF AI CELL-`;

    // Insert new code into current cell
    codeMirror.setValue(formattedCode);

    // Find and click the corresponding code execution button
...
function createAiSettingsModal() {
    ...
    const savedVisibilityPref = localStorage.getItem('aiCellsHiddenPreference');
    if (savedVisibilityPref !== null) {
        aiCellsHidden = JSON.parse(savedVisibilityPref);
    }
    // --- End Load ---

    const currentApiKey = typeof API_KEY !== 'undefined' ? API_KEY : ''; // Ensure API_KEY is defined
    ...
    removeExistingModal(); // Ensure no duplicates
    ...
    styleElement.textContent = `
        ...
        .modal-content {
            ...
            margin: 10% auto; /* Centered, slightly lower */
        ...
        .update-button { /* Specific class for Update button */
        ...
        .modal-section {
            margin-top: 25px;
            padding-top: 20px;
            border-top: 1px solid #e5e5e5; /* Slightly more visible separator */
        }
        .modal-section h4 {
            margin-top: 0;
        ...
        .danger-zone-text { /* Class for the paragraph in danger zone */
             font-size: 0.85em;
             color: #777;
             margin-top: 8px;
        ...
        .warning-message {
            ...
            margin: 25px 0 0 0; /* Margin top, remove others */
            border-radius: 4px;
            font-size: 0.9em;
        }
        .warning-message strong {
         ...
         .warning-message p {
            margin: 0; /* Reset paragraph margin inside warning */
            color: inherit; /* Inherit color */
            font-size: 1em; /* Reset font size */
         }

         /* Button Grouping (Optional - for layout) */
         .button-group {
            margin-top: 20px;
            text-align: right; /* Align buttons to the right */
         }
         .button-group button {
            margin-left: 10px; /* Space between buttons when right-aligned */
            margin-right: 0;
         }
    `;
    ...
    // --- Updated Modal HTML ---
    // Added specific classes to buttons and paragraphs for better targeting
    ...
    modalDiv.innerHTML = `
        <div class="modal-content">
            ...
            <div class="input-group">
                ...
                <input type="text" id="newKey" value="${currentApiKey}" placeholder="Enter your Mistral API key">
                <p style="text-align: center;">Obtain a free Mistral API key from the <a href="https://console.mistral.ai/api-keys" target="_blank" rel="noopener noreferrer">Mistral AI Platform</a>.</p>
            </div>
             <!-- Update Button (moved near related fields) -->
            <button class="update-button" onclick="updateSettings()">Update API/Model/Language</button>


            <!-- AI Cell Visibility Section -->
            <div class="modal-section">
                <h4>AI Cell Management</h4>
                <label class="checkbox-label">
                    <input type="checkbox" id="hideAiCellsCheckbox" onchange="handleAiVisibilityToggle(this)" ${aiCellsHidden ? 'checked' : ''}>
                    Hide AI Generated Cells
                </label>
                <p>Toggle the visibility of code cells generated using the AI features.</p>
            </div>

            <!-- Delete AI Cells Section -->
            <div class="modal-section">
                 <button class="delete-button" onclick="deleteAllAiCells()">Delete All AI Cells</button>
                 <p class="danger-zone-text">This action is permanent and cannot be undone.</p>
            </div>
            ...
            <div class="warning-message">
                ...
                <p>AI cells contain API keys. Delete all AI cells if necessary.</p>
...
function updateSettings() {
    ...
    // API Key is NOT saved to localStorage for security

    API_KEY = newKey;
    CURRENT_MODEL = selectedModel;
    CURRENT_LANGUAGE = selectedLanguage;

    // No need to remove modal here, let user close it manually
    // removeExistingModal();
    console.log(`Updated! Model: ${CURRENT_MODEL}, API Key: ${API_KEY ? '******' : 'Not Set'}, Language: ${CURRENT_LANGUAGE}`);

    // Refresh control bars to reflect potential API key changes (show/hide AI buttons)
    removeAllControlBars();
    initializeCells();

    alert("API Key, Model, and Language settings updated."); // Give feedback
}
...
/**
 * Checks if a given code cell is an AI-generated cell based on markers.
 * @param {HTMLElement} cell - The code cell element (div.nb-cell.nb-code-cell).
 * @returns {boolean} - True if it's an AI cell, false otherwise.
 */
function isAiCell(cell) {
    ...
    // Prioritize checking the CodeMirror instance if available
    const cmEditor = cell.querySelector('.CodeMirror');
    if (cmEditor && cmEditor.CodeMirror) {
        const code = cmEditor.CodeMirror.getValue();
        // Check for both start and end markers for robustness, though start is primary
        return code.includes("# -START OF AI CELL-") || code.includes("# -END OF AI CELL-");
    }

    // Fallback: Check the initial script tag content if CodeMirror isn't ready
    // This might be less reliable if the content was changed before CM init
    const scriptTag = cell.querySelector('script[type="text/x-sage"]');
    if (scriptTag) {
        return scriptTag.textContent.includes("# -START OF AI CELL-") || scriptTag.textContent.includes("# -END OF AI CELL-");
    }

    // Fallback: Check simplified pre/code structure if present
    const codeContentElement = cell.querySelector('.nb-input pre code') || cell.querySelector('.nb-input pre');
    if (codeContentElement) {
        return codeContentElement.textContent.includes("# -START OF AI CELL-") || codeContentElement.textContent.includes("# -END OF AI CELL-");
    }


    console.warn("Could not determine content for AI cell check:", cell);
    return false; // Default to false if content can't be checked
}

/**
 * Toggles the visibility of all AI code cells in the notebook.
 */
function toggleAiCellsVisibility() {
    const codeCells = document.querySelectorAll('.nb-cell.nb-code-cell');
    const newStateHidden = !aiCellsHidden; // Determine the target state

    codeCells.forEach(cell => {
        if (isAiCell(cell)) {
            // Apply the new state to AI cells
            cell.style.display = newStateHidden ? 'none' : ''; // Use '' to revert to default display
        }
    });

    // Update the global state
    aiCellsHidden = newStateHidden;

    // Update the button text to reflect the *next* action
    const toggleAiButton = document.getElementById('toggleAiCellsButton');
    if (toggleAiButton) {
        toggleAiButton.textContent = aiCellsHidden ? 'Show AI Cells' : 'Hide AI Cells';
        toggleAiButton.title = aiCellsHidden ? 'Show cells generated by AI' : 'Hide cells generated by AI';
    }

    console.log(`AI cells are now ${aiCellsHidden ? 'hidden' : 'visible'}`);
}

/**
 * Applies the visibility state (show/hide) to all AI code cells.
 * @param {boolean} shouldHide - True to hide AI cells, false to show them.
 */
function applyAiCellVisibility(shouldHide) {
    const codeCells = document.querySelectorAll('.nb-cell.nb-code-cell');
    let count = 0;
    codeCells.forEach(cell => {
        if (isAiCell(cell)) {
            cell.style.display = shouldHide ? 'none' : ''; // Use '' to revert to default
            count++;
        }
    });
    console.log(`${count} AI cells are now ${shouldHide ? 'hidden' : 'visible'}`);
}

/**
 * Handles the change event from the "Hide AI Cells" checkbox in the modal.
 * @param {HTMLInputElement} checkbox - The checkbox element that triggered the event.
 */
function handleAiVisibilityToggle(checkbox) {
    const shouldHide = checkbox.checked;

    // Update global state
    aiCellsHidden = shouldHide;

    // Persist the preference
    localStorage.setItem('aiCellsHiddenPreference', JSON.stringify(aiCellsHidden));

    // Apply the change visually
    applyAiCellVisibility(aiCellsHidden);
}

// Keep the isAiCell function as defined previously
// function isAiCell(cell) { ... }

// --- Add this call on page load to apply the initial state ---
document.addEventListener('DOMContentLoaded', () => {
    // Load preference and apply initial visibility state
    const savedVisibilityPref = localStorage.getItem('aiCellsHiddenPreference');
    if (savedVisibilityPref !== null) {
        aiCellsHidden = JSON.parse(savedVisibilityPref);
        // Apply the loaded state *after* cells are likely initialized
    ...
    // Keep other DOMContentLoaded logic if any (like removeDuplicateStyles)
    removeDuplicateStyles();
});

/**
 * Permanently deletes all AI-generated code cells from the notebook after confirmation.
 */
function deleteAllAiCells() {
    const confirmation = confirm(
        "⚠️ Are you sure you want to permanently delete ALL AI-generated code cells?\n\n" +
    ...
    if (confirmation) {
        const codeCells = document.querySelectorAll('.nb-cell.nb-code-cell');
        let deletedCount = 0;
        const cellsToRemove = []; // Collect cells to remove first
        ...
        if (deletedCount > 0) {
            console.log(`Deleted ${deletedCount} AI cells.`);
            alert(`Successfully deleted ${deletedCount} AI cells.`);
            // Reprocess the notebook to update numbering, etc.
            // Use a timeout to ensure removal is complete before reprocessing
            setTimeout(() => {
                reprocessNotebook();
            }, 100);
        } else {
            alert("No AI cells found to delete.");
        }

        // Optionally close the modal after deletion
        // removeExistingModal();
    } else {
        console.log("AI cell deletion cancelled by user.");
    }
}
