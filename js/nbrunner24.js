...
function makeMenu() {
    ...
    $("head").first().append('<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/JupyterPER/SageMathAINotebooks@main/css/nbplayer24.css"'), $("body").first().append('<script src="custom.js"><\/script>');
...
// ============================================================
// AI USAGE LOGS (stored in the saved HTML notebook only)
// ============================================================

// The logs are session-memory data while the notebook is open.
// They are serialized into the downloaded HTML only by saveHtml().
if (!Array.isArray(window.aiLogsHistory)) {
    window.aiLogsHistory = [];
}

// Controls whether the AI Settings section is shown in the Settings modal.
if (typeof window.aiSettingsVisible === 'undefined') {
    window.aiSettingsVisible = true;
}

function isAiLoggingEnabled() {
    // Prefer the existing aiLogs variable, which may be provided by settings.js.
    if (typeof aiLogs !== 'undefined') {
        return !!aiLogs;
    }

    return !!window.aiLogs;
}

function getAiLogNotebookFileName() {
    const rawName = extractFilenameBaseFromH1() || playerConfig?.name || 'Untitled_Notebook';

    // Remove common timestamp suffixes if the H1 itself contains one.
    // Examples removed:
    // _2026_08_26_14_22_31
    // -2026-08-26-14-22-31
    // _2026-08-26_14-22-31
    return String(rawName)
        .replace(
            /(?:[_-])\d{4}[_-]\d{2}[_-]\d{2}(?:[_-]\d{2}){0,3}$/u,
            ''
        )
        .replace(/[_-]+$/u, '') || 'Untitled_Notebook';
}

function getAiLogCodeFromCell(cell) {
    if (!cell) return '';

    const sageCm = cell.querySelector('.sagecell_input .CodeMirror');
    if (sageCm && sageCm.CodeMirror) {
        return String(sageCm.CodeMirror.getValue() || '')
            .replace(/[\u200B\u200C\u200D\uFEFF]/g, '');
    }

    const cm = cell.querySelector('.CodeMirror');
    if (cm && cm.CodeMirror) {
        return String(cm.CodeMirror.getValue() || '')
            .replace(/[\u200B\u200C\u200D\uFEFF]/g, '');
    }
...
// The "focus cell" means the nearest preceding code cell.
// This is especially relevant to AI Complete and AI Educate.
function getAiLogFocusCellContent(cell) {
    let previous = cell ? cell.previousElementSibling : null;

    while (previous) {
        if (previous.classList?.contains('nb-code-cell')) {
            return getAiLogCodeFromCell(previous);
        }
...
function getAiAgentLabel(aiCommand) {
    const labels = {
        AI_complete: 'AI Complete',
        AI_format: 'AI Format',
        AI_explain: 'AI Explain',
        AI_edu: 'AI Educate',
        AI_vision_ocr: 'AI Vision (OCR)',
        AI_vision_code: 'AI Vision (Code)',
        AI_vision_custom: 'AI Vision (Custom)'
    };

    return labels[aiCommand] || aiCommand || 'AI';
}

/*
 * Appends one AI-usage record.
 *
 * `comparisonContent` is used only to calculate repeated-use count.
 * For normal agents it is normally the same as cellContent.
 * For AI Vision, cellContent is intentionally the AI description/output,
 * while comparisonContent remains the original target-cell content.
 */
function appendAiUsageLog({
    aiCommand,
    cellContent,
    focusCellContent = '',
    sourceCellContent = '',
    ...
    const normalizedComparisonContent = String(
        comparisonContent || sourceCellContent || cellContent || ''
    );

    const matchingEntries = window.aiLogsHistory.filter(entry =>
        entry &&
        entry.aiCommand === aiCommand &&
        String(entry.comparisonContent || entry.sourceCellContent || entry.cellContent || '') === normalizedComparisonContent
    );

    const entry = {
        timestamp: new Date().toISOString(),
        fileName: getAiLogNotebookFileName(),
        agent: getAiAgentLabel(aiCommand),
        aiCommand: aiCommand,
        cellContent: String(cellContent || ''),
        count: matchingEntries.length + 1,

        // Kept so repeat counting remains correct even for Vision,
        // where cellContent stores the returned description/output.
        comparisonContent: normalizedComparisonContent
    };

    if (focusCellContent) {
        entry.focusCellContent = String(focusCellContent);
    }

    if (sourceCellContent) {
        entry.sourceCellContent = String(sourceCellContent);
    }
    ...
    window.aiLogsHistory.push(entry);
}

function clearAiLogsHistory() {
    const currentCount = Array.isArray(window.aiLogsHistory)
        ? window.aiLogsHistory.length
        : 0;

    if (currentCount === 0) {
        alert('AI Cache is already empty.');
        return;
    }

    const confirmed = confirm(
        `Clear AI Cache?\n\nThis removes ${currentCount} AI usage log entr${currentCount === 1 ? 'y' : 'ies'} from the current notebook session.\n\nThe change becomes permanent in the notebook only after you save it.`
    );

    if (!confirmed) return;

    window.aiLogsHistory = [];
    alert('AI Cache cleared. Save the notebook to store the cleared cache.');
}

function setAiSettingsVisible(visible) {
    window.aiSettingsVisible = !!visible;

    // Rebuild the modal immediately, so the AI Settings section appears/disappears.
...
function saveHtml() {
    ...
    const currentAiLogs = typeof aiLogs !== 'undefined'
        ? JSON.stringify(!!aiLogs)
        : JSON.stringify(!!window.aiLogs);

    const currentAiSettingsVisible = JSON.stringify(
        typeof window.aiSettingsVisible === 'undefined'
            ? true
            : !!window.aiSettingsVisible
    );

    // Important: escape "<" so a log containing "</script>" cannot break the saved HTML.
    const serializedAiLogsHistory = JSON.stringify(
        Array.isArray(window.aiLogsHistory) ? window.aiLogsHistory : []
    ).replace(/</g, '\\u003c')
     .replace(/>/g, '\\u003e')
     .replace(/&/g, '\\u0026');
    ...
    const htmlContent = "<!DOCTYPE html>\n<html>\n<head>" +
        ...
        '<script src="https://cdn.jsdelivr.net/gh/JupyterPER/SageMathAINotebooks@main/js/nbrunner24.js"></script>\n' +
        '<script src="https://cdn.jsdelivr.net/gh/JupyterPER/SageMathAINotebooks@main/js/nbaiengine23.js"></script>\n' +
        '<script src="https://cdn.jsdelivr.net/gh/JupyterPER/SageMathAINotebooks@main/js/nbaipriming22.js"></script>\n' +
        ...
        '  let aiLogs=' + currentAiLogs + ';\n' +
        '  window.aiLogs=' + currentAiLogs + ';\n' +
        '  window.aiSettingsVisible=' + currentAiSettingsVisible + ';\n' +
        '  window.aiLogsHistory=' + serializedAiLogsHistory + ';\n' +
        '  makeMenu();\n' +
        '  localize();\n' +
...
function addControlPanel() {
    ...
    function showImportExportPopup() {
        ...
        const exportButtons = [
            ...
            makeOptionButton('HTML (Jupyter webpage)', () => downloadNotebookAsHTML()),
            makeOptionButton('PDF (print dialog)',     () => printNotebookAsPDF()),
    ...
    if (linkElement) {
        linkElement.href = "https://cdn.jsdelivr.net/gh/JupyterPER/SageMathAINotebooks@main/css/nbplayer24.css";
    }
...
// ============================================================
// VERSION 24 — Static HTML / PDF export (ipynb2html + hljs)
// ============================================================
// Exports the notebook as a fully self-contained Jupyter-style HTML
// page (rendered with ipynb2html, all CSS embedded) and triggers a
// file download. PDF export renders the same document into a blob
// URL and opens the browser print dialog on it ("Save as PDF").
// The HTML/PDF pipeline never touches the live page DOM.
//
// NOTE: the old NB_NOTEBOOK_EXPORT_CSS constant from version 24 has
// been deleted. NOTEBOOK_EXPORT_CSS below is the single source of
// truth for export styling, and it now matches the .notebook-export
// wrapper class used by buildNotebookHtmlDocument().

// --- Vendor stylesheets to inline into every export -----------------

const NOTEBOOK_EXPORT_CSS_HREFS = [
  'https://cdn.jsdelivr.net/npm/ipynb2html@0.4.0-rc.1/dist/notebook.min.css',
...
// --- Export-specific stylesheet (inlined verbatim) ------------------


const NOTEBOOK_EXPORT_CSS = `
  *,
  *::before,
  *::after {
  ...
  .notebook-export {
    --notebook-text-font: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;

    max-width: 1100px;
    margin: 0 auto;
  }

  .notebook-export > .nb-notebook {
    padding-left: 6.5em;
  }

  /*
   * The selected text font only applies to non-code cells.
   * Code cells, code fragments, and raw cells remain monospace.
   */

  .notebook-export .nb-cell:not(.nb-code-cell):not(.nb-raw-cell) {
    font-family: var(--notebook-text-font);
  }

  .notebook-export .nb-cell:not(.nb-code-cell) code,
  .notebook-export .nb-cell:not(.nb-code-cell) pre {
    font-family:
  ...
  /*
   * Standard notebook table styling.
   */
  ...
  .notebook-export thead th {
    background: #e2e8f0;
    color: #0f172a;
    font-weight: 650;
  }

  .notebook-export tbody tr:nth-child(even) > th,
  .notebook-export tbody tr:nth-child(even) > td {
    background: #f8fafc;
  }

  .notebook-export tbody tr:hover > th,
  .notebook-export tbody tr:hover > td {
    background: #eff6ff;
  }

  .notebook-export caption {
  ...
  .notebook-export .nb-source::before,
  .notebook-export .nb-output::before {
    left: -6.5em;
    width: 6em;
    font-weight: 600;
  }
  ...
  .notebook-export .nb-source > pre {
    background: #f8fafc;
    border-color: #cbd5e1;
    border-radius: 5px;
  }

  .notebook-export .nb-source code {
    display: block;
    color: #1f2937;
    font-family:
      ui-monospace, SFMono-Regular, Menlo, Monaco,
  ...
  /*
   * More vivid Python/SageMath syntax highlighting.
   * Placed after the default theme so these colors override it.
   */

  .notebook-export .hljs-keyword,
  .notebook-export .hljs-selector-tag,
  .notebook-export .hljs-literal {
    color: #7c3aed;
    font-weight: 650;
  }

  .notebook-export .hljs-built_in,
  .notebook-export .hljs-type {
    color: #0369a1;
    font-weight: 600;
  }

  .notebook-export .hljs-title,
  .notebook-export .hljs-title.function_,
  .notebook-export .hljs-title.class_ {
    color: #b45309;
    font-weight: 650;
  }

  .notebook-export .hljs-string,
  .notebook-export .hljs-regexp {
    color: #15803d;
  }

  .notebook-export .hljs-number {
    color: #c2410c;
  }

  .notebook-export .hljs-comment,
  .notebook-export .hljs-quote {
    color: #64748b;
    font-style: italic;
  }

  .notebook-export .hljs-variable,
  .notebook-export .hljs-template-variable,
  .notebook-export .hljs-attr,
  .notebook-export .hljs-attribute {
    color: #be123c;
  }

  .notebook-export .hljs-params {
    color: #1e40af;
  }

  .notebook-export .hljs-meta,
  .notebook-export .hljs-meta .hljs-keyword {
    color: #a21caf;
  }

  .notebook-export .hljs-symbol,
  .notebook-export .hljs-bullet {
    color: #0f766e;
  }

  .notebook-export .hljs-subst {
    color: #1f2937;
  }
  ...
  @media print {
    ...
    /*
     * Browsers are normally allowed to remove background colors
     * when printing. This requests that code-cell and table
     * backgrounds be preserved. Browser print settings can still
     * take precedence.
     */
    ...
    /*
     * Do not include hover coloring in printed output.
     */
    .notebook-export tbody tr:hover > th,
    .notebook-export tbody tr:hover > td {
    ...
    /*
     * Preserve the left prompt gutter in PDF output.
     */
...
const NB_EXPORT_LIB_URLS = {
    hljs:       'https://cdn.jsdelivr.net/gh/highlightjs/cdn-release@11.11.1/build/highlight.min.js',
    ipynb2html: 'https://cdn.jsdelivr.net/npm/ipynb2html@0.4.0-rc.1/dist/ipynb2html-full.min.js'
};

function loadScriptOnce(url, globalName) {
    return new Promise((resolve, reject) => {
        // Already available (page loaded it, or a previous export did)?
        if (typeof window[globalName] !== 'undefined') return resolve();
        // A <script> tag for it is already in flight? Wait for its load/error.
        const existing = document.querySelector('script[data-nbexport-lib="' + globalName + '"]');
        if (existing) {
            existing.addEventListener('load', () => resolve());
            existing.addEventListener('error', () => reject(new Error('Failed to load ' + globalName + ' from ' + url)));
            return;
        }
        const script = document.createElement('script');
        script.src = url;
        script.async = true;
        script.dataset.nbexportLib = globalName;
...
async function ensureExportLibraries() {
    await loadScriptOnce(NB_EXPORT_LIB_URLS.hljs, 'hljs');
    await loadScriptOnce(NB_EXPORT_LIB_URLS.ipynb2html, 'ipynb2html');
    if (typeof window.ipynb2html === 'undefined' || typeof window.hljs === 'undefined') {
        throw new Error('Export libraries did not initialise correctly.');
    }
}
...
function getNotebookIpynbString() {
    return collectNotebookAsIPYNB(true).then(JSON.stringify);
}

function getExportFilenameBase() {
    return extractFilenameBaseFromH1() || 'SageMath_notebook';
}
...
function downloadBlob(blob, filename) {
  ...
  // Do not revoke immediately — some browsers cancel the download.
  setTimeout(() => URL.revokeObjectURL(url), 30_000);
}

// --- Vendor CSS inlining --------------------------------------------
//
// External <link> tags are useless in a downloaded standalone HTML
// file opened via file:// (mixed-content/cors quirks aside, they can
// also race the print dialog). Everything is fetched once and inlined.
// Relative url(...) references (KaTeX fonts!) are rewritten to
// absolute URLs so the embedded CSS still resolves its assets.

function makeCssUrlsAbsolute(cssText, cssUrl) {
  return cssText.replace(
    /url\(\s*(['"]?)([^'")]+)\1\s*\)/gi,
    (fullMatch, quote, rawUrl) => {
      const url = rawUrl.trim();

      // Already self-contained or already absolute.
      if (
        url.startsWith('data:') ||
        url.startsWith('blob:') ||
        url.startsWith('#') ||
      ...
      return `url("${new URL(url, cssUrl).href}")`;
    }
  );
}

async function fetchStylesheetText(cssUrl) {
  ...
  if (!response.ok) {
    throw new Error(
      `Could not load export stylesheet: ${cssUrl} (${response.status})`
    );
  }

  const cssText = await response.text();

  // Prevent a literal </style> inside the fetched CSS from ending
  // the <style> tag early in the generated document.
  return makeCssUrlsAbsolute(cssText, cssUrl)
    .replace(/<\/style/gi, '<\\/style');
}

let notebookExportVendorCssPromise = null;

function getNotebookExportVendorCss() {
  if (!notebookExportVendorCssPromise) {
    notebookExportVendorCssPromise = Promise.all(
      NOTEBOOK_EXPORT_CSS_HREFS.map(fetchStylesheetText)
    ).then(cssParts => cssParts.join('\n\n'));
  }

  return notebookExportVendorCssPromise;
}

// --- Document assembly ----------------------------------------------

async function buildNotebookHtmlDocument(bodyHtml) {
  const vendorCss = await getNotebookExportVendorCss();

  const filenameBase = getExportFilenameBase();
  const title = escapeHtml(filenameBase.replace(/_/g, ' '));
...
<head>
  ...
  <!-- All export CSS is embedded so the downloaded HTML retains styling. -->
  <style id="notebook-export-vendor-css">
${vendorCss}
  </style>

  <style id="notebook-export-custom-css">
${NOTEBOOK_EXPORT_CSS}
  </style>
</head>
<body>
...
async function renderNotebookExportBody() {
  await ensureExportLibraries();

  const notebook = await collectNotebookAsIPYNB(true);
  const rendered = window.ipynb2html.render(notebook);
...
async function downloadNotebookAsHTML() {
  const bodyHtml = await renderNotebookExportBody();
  const fullHtml = await buildNotebookHtmlDocument(bodyHtml);

  downloadBlob(
    new Blob([fullHtml], { type: 'text/html;charset=utf-8' }),
    getExportFilenameBase() + '.html'
  );
}

async function printNotebookAsPDF() {
  const bodyHtml = await renderNotebookExportBody();
  const fullHtml = await buildNotebookHtmlDocument(bodyHtml);
  ...
  printWindow.addEventListener('load', () => {
    // Show the warning in the original page.
    showPrintWarning();

    // Allow the notice to render before opening the print dialog.
    setTimeout(() => {
      printWindow.focus();
...
function showPrintWarning() {
  ...
  printWarning = document.createElement('div');
  printWarning.textContent =
    'Print to PDF is active. This page will be temporarily unresponsive until printing is finished or canceled.';
