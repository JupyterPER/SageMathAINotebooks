...
function createAiSettingsModal() {
    ...
    const currentAiLogsEnabled = typeof aiLogs !== 'undefined'
        ? !!aiLogs
        : !!window.aiLogs;

    const aiSettingsSectionVisible = typeof window.aiSettingsVisible === 'undefined'
        ? true
    ...
    const aiHtml = `
        <label class="toggle-row">
            <input type="checkbox" id="aiLogsToggle" ${currentAiLogsEnabled ? 'checked' : ''}>
            <span class="toggle-text">Enable AI usage cache
                <small>Stores AI usage records in the notebook when it is saved. The cache does not store API keys or Vision images.</small>
            </span>
        </label>
    ...
    const notebookHtml = `
        <div class="settings-subtitle">Settings Visibility</div>

        <label class="toggle-row">
            <input type="checkbox" id="aiSettingsVisibleToggle"
                   ${aiSettingsSectionVisible ? 'checked' : ''}
                   onchange="setAiSettingsVisible(this.checked)">
            <span class="toggle-text">Show AI Settings section
                <small>Controls whether the AI Settings accordion section is visible in this modal.</small>
            </span>
        </label>
    ...
    const fileHtml = `
        ...
        <div class="input-group">
            <label>AI Cache:</label>
            <p class="danger-zone-text">
                Clears the in-memory AI usage history for this notebook. The cleared state
                is written into the notebook only after saving.
            </p>
            <button type="button" class="danger-btn" onclick="clearAiLogsHistory()">
                Clear AI Cache
            </button>
        </div>
    ...
    <div class="modal-content">
        ...
        <div class="modal-body" id="settingsScrollArea">
            ...
            ${aiSettingsSectionVisible ? sec('ai', 'AI Settings', aiHtml) : ''}
...
function updateAiSettings() {
    ...
    // AI settings may not exist in the modal when the AI section is hidden.
    const newKey = getValue(
        'newKey',
        typeof API_KEY !== 'undefined' ? API_KEY : ''
    );

    const newApiUrl = getValue(
        'apiUrlInput',
        typeof API_URL !== 'undefined' ? API_URL : ''
    );

    const selectedModel = getValue(
        'modelInput',
        typeof CURRENT_MODEL !== 'undefined' ? CURRENT_MODEL : ''
    );

    const selectedLanguage = getValue(
        'languageInput',
        typeof CURRENT_LANGUAGE !== 'undefined' ? CURRENT_LANGUAGE : ''
    );

    const customContext = getValue(
        'customContextInput',
        typeof CUSTOM_CONTEXT !== 'undefined' ? CUSTOM_CONTEXT : ''
    );

    // AI usage logging
    const aiLogsToggle = document.getElementById('aiLogsToggle');

    if (aiLogsToggle) {
        aiLogs = !!aiLogsToggle.checked;
        window.aiLogs = aiLogs;
    }
    ...
    // SageCell URL
    const sagecellUrlInput = document.getElementById('sagecellUrlInput');

    if (sagecellUrlInput) {
        let sagecellUrl = sagecellUrlInput.value.trim();

        if (sagecellUrl && !sagecellUrl.endsWith('/')) {
            sagecellUrl += '/';
            sagecellUrlInput.value = sagecellUrl;
        }
    ...
    // Custom CSS
    const cssInput = document.getElementById('customCssInput');
    const customCssValue = cssInput
        ? cssInput.value
        : (typeof CUSTOM_CSS !== 'undefined' ? CUSTOM_CSS : '');
    ...
    console.log(
        ...
        `AI logs: ${typeof aiLogs !== 'undefined' && aiLogs ? 'Enabled' : 'Disabled'}, ` +
        `Run delay: ${RUN_DELAY} ms, ` +
        `SageCell: ${SAGECELL_URL}, ` +
        `Custom CSS: ${CUSTOM_CSS ? CUSTOM_CSS.length + ' chars' : 'None'}`
    );

    // Refresh control bars because API-key availability may have changed.
...
async function formatAndLoadCodeIntoCell(cell, aiCommand, currentModel, apiKey, currentLanguage, currentCustomContext) {
    ...
    // Capture the exact state at the moment the user requested the AI action.
    // This is intentionally done before the request starts.
    const userCode = cm.getValue();
    const focusCellContent = (
        aiCommand === 'AI_complete' || aiCommand === 'AI_edu'
    ) && typeof getAiLogFocusCellContent === 'function'
        ? getAiLogFocusCellContent(cell)
        : '';
    ...
    const loadingCell = addMarkdownCell(
        cell,
        "below",
        '<div class="AIstatus">AI agent is working </div>'
    );
    ...
    try {
        ...
        // Log only after a successful response has been received.
        // No API key, full prompt, or previous-cell context is written into the log.
        if (typeof appendAiUsageLog === 'function') {
            appendAiUsageLog({
                aiCommand: aiCommand,
                cellContent: userCode,
                focusCellContent: focusCellContent,
                comparisonContent: userCode
            });
        }
        ...
        const preview = loadingCell.querySelector(".markdown-preview");
        if (preview) {
            preview.textContent = `⚠️ Error: ${err.message}`;
        }
    }
}
...
async function submitVisionRequest() {
    ...
    // Capture log-relevant information before the modal is removed.
    var sourceCellContent = '';
    var cmBeforeRequest = cell.querySelector(".CodeMirror");

    if (cmBeforeRequest && cmBeforeRequest.CodeMirror) {
        sourceCellContent = cmBeforeRequest.CodeMirror.getValue();
    }

    var uploaded = visionState.images.some(function(img) {
        return img.source === 'uploaded';
    });

    var drawn = visionState.images.some(function(img) {
        return img.source === 'drawn';
    });

    var visionLogMetadata = {
        mode: mode,
        imageCount: visionState.images.length,
        uploaded: uploaded,
        drawn: drawn
    };

    // Collect image payload before closing modal.
    var imageDataUrls = visionState.images.map(function(img) {
        return img.dataUrl;
    });
    ...
    if (currentLanguage && currentLanguage.trim()) {
        if (mode === 'ocr') {
            userPrompt +=
                '\n\nIMPORTANT: If you add any notes or descriptions (not the transcribed text itself), write them in ' +
                currentLanguage + '.';
        } else {
    ...
    var loadingCell = addMarkdownCell(
        cell,
        "below",
        '<div class="AIstatus">AI agent is processing your image </div>'
    );

    try {
        var aiText = await callAiVisionApi(
            API_URL,
            CURRENT_MODEL,
            systemPrompt,
            userPrompt,
        ...
        /*
         * Vision intentionally logs the returned response/description rather
         * than image data. This avoids embedding potentially large base64
         * images in notebook history.
         */
        if (typeof appendAiUsageLog === 'function') {
            appendAiUsageLog({
                aiCommand: aiCommand,
                cellContent: aiText,
                sourceCellContent: sourceCellContent,
                comparisonContent: sourceCellContent,
                vision: visionLogMetadata
            });
        }
...
function getCurrentSettings() {
    ...
    return {
        ...
        aiSettingsVisible:   typeof window.aiSettingsVisible !== "undefined"
            ? !!window.aiSettingsVisible
            : true,
    };
}
...
window.aiSettingsVisible = ${typeof s.aiSettingsVisible !== "undefined" ? JSON.stringify(!!s.aiSettingsVisible) : "true"}; // show/hide AI Settings section
...
function applyImportedSettings(s) {
    ...
    if ('aiLogs' in s) aiLogs = !!s.aiLogs;
    if ('aiSettingsVisible' in s) {
        window.aiSettingsVisible = !!s.aiSettingsVisible;
    }
    ...
    modal.innerHTML = `
        <div class="modal-content confirm-modal-content" style="max-width:480px;">
            ...
            <ul>
                ...
                <li>UI state (toggle eval buttons, full width, AI Settings visibility)</li>
                <li>AI usage cache/history</li>
            </ul>
