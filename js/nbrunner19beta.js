...
function addControlPanel() {
    ...
    exportButton.onclick = function() {
        showFormatPopup('Export Format', [
            ...
            { label: 'SAGE (code only)', action: downloadAllCodeAsSage },
            { label: 'SAGE (minified / base64)', action: downloadAllCodeAsMinifiedSage }
        ]);
    };
    ...
    function showFormatPopup(title, options) {
        ...
        options.forEach((opt, index) => {
            ...
            btn.style.backgroundColor = 'var(--main-color, #4CAF50)';
    ...
    const toggleEvalButton = document.createElement('button');
    toggleEvalButton.id = 'toggleEvalButton';
    toggleEvalButton.innerHTML = iconDictionaryNavbar["toggleEval"];
    toggleEvalButton.title = 'Toggle Eval Buttons Visibility';
    toggleEvalButton.onclick = toggleEvalButtons;
    ...
    bulkActionsButton.title = 'Bulk Cell Actions / Merge (show / hide checkboxes)';
    bulkActionsButton.onclick = toggleBulkCheckboxes;

    navbar.insertBefore(toggleEvalButton, navbar.firstChild);
    ...
    const linkElement = document.querySelector('link[href="https://dahn-research.eu/nbplayer/css/nbplayer18.css"]');
    if (linkElement) {
        linkElement.href = "https://cdn.jsdelivr.net/gh/JupyterPER/SageMathAINotebooks@main/css/nbplayer18.css";
    }
...
const iconDictionaryNavbar = {
    ...
    toggleEval: `<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><text x="12" y="16" fill="currentColor" stroke="none" font-size="10" font-weight="bold" font-family="sans-serif" text-anchor="middle">Ex</text></svg>`,
...
const iconDictionary = {
    ...
    aiComplete: `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none">
    ...
    runUpTo: `<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14">
    ...
    splitCell: `<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 17 17" fill="currentColor">
...
function duplicateCell(cell) {
    ...
    if (cell.classList.contains('nb-markdown-cell')) {
        ...
        // const originalHeight = (origCM && origCM.getAttribute('data-original-height'));
...
function createHelpModal() {
    ...
    const shortcuts = [
        ...
        { keys: 'Alt+V', description: 'Duplicate cell' },
...
// let isFullWidthState = false;
// let toggleEvalBtnsState = false;

// Inject (once) the stylesheet that hides eval buttons when body has the class.
...
// Single source of truth: reflect the boolean onto the body class.
...
function applyFullWidthState() {
    ...
    if (main) {
        main.style.maxWidth = isFullWidthState ? '90%' : '750px';
    }
}

// Apply whatever the current globals are (used on load / after restore).
...
function toggleEvalButtons() {
    toggleEvalBtnsState = !toggleEvalBtnsState;
    applyEvalButtonsState();
}
...
function flipMaxWidth() {
    ...
    isFullWidthState = !isFullWidthState;
    applyFullWidthState();
}
...
/* ============================================================
   NOTEBOOK EDIT HISTORY  +  UNDO / REDO   (session-only)
...
(function () {
    ...
    function serializeCell(cell) {
        if (!cell) return { type: 'code', content: '' };
        if (cell.classList.contains('nb-markdown-cell'))
    ...
    function insertCellsAt(index, dataArray) {
        ...
        let refNode;
        if (index >= cells.length) refNode = cells.length ? cells[cells.length - 1].nextSibling : null;
        else refNode = cells[index];
    ...
    function moveCellByIndex(from, to) {
        ...
        parent.insertBefore(node, remaining[to] || null);
        removeSageCellNumbering(); addSageCellNumbering();
    }

    // full-worksheet snapshot / restore (used by bulk ops)
    ...
    const History = {
        ...
        clear() {
            ...
            this.updateUI(); this.refreshPanel();
        },
    ...
    addFiveCells = function (referenceCell, position) {
        ...
        const idx = (position === 'above') ? refIdx : refIdx + 1;
        History.push({
            label: 'Add 5 code cells ' + position,
            meta: { op: 'add', index: idx, count: 5 },
            data: Array.from({ length: 5 }, () => ({ type: 'code', content: '' })),
            undo() { this.data = removeCellsAt(idx, 5); },   // capture typed content
            redo() { insertCellsAt(idx, this.data); }
        });
    };
    ...
    moveCell = function (cell, direction) {
        ...
        if (from === to) return;             // hit top/bottom — nothing happened
        History.push({
    ...
    convertToMarkdown = function (codeCell) {
        ...
        History.push({
            ...
            undo() { this.after = replaceCellAt(idx, this.before); },
            redo() { this.before = replaceCellAt(idx, this.after); }
        });
    };
    ...
    splitCellAtCursor = function (cell) {
        ...
        History.push({
            ...
            undo() {
                ...
                this.secondData = removeCellsAt(idx + 1, 1)[0];
    ...
    // ---------- wrap the bulk operations (snapshot based) --------------------
    ...
    if (typeof bulkMove === 'function')         bulkMove         = wrapBulk(bulkMove,         'Bulk move');
    ...
    function initUI(attempt = 0) {
        ...
        histBtn.id = 'historyButton'; histBtn.title = 'Edit History'; histBtn.innerHTML = ICONS.hist;
        histBtn.onclick = () => {
            buildPanel();
        ...
        navbar.insertBefore(histBtn, navbar.firstChild);
